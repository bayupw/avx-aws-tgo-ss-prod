...
resource "aws_security_group" "prd_banking_instance_sg" {
  count       = var.create_ec2 ? 1 : 0
  name        = "prd-banking/sg-instance"
...
resource "aws_instance" "prd_banking_instance" {
  count                       = var.create_ec2 ? 1 : 0
  ...
  vpc_security_group_ids      = [aws_security_group.prd_banking_instance_sg[0].id]
  associate_public_ip_address = true

  tags = {
    Name = "prd-banking-instance"
  }
}
...
resource "aws_instance" "prd_it_service_instance" {
  ...
  instance_type               = "t2.micro"
  key_name                    = var.key_name == null ? aws_key_pair.instance_key_pair[0].key_name : var.key_name
  subnet_id                   = module.prd_it_service_vpc.public_subnets[0]
